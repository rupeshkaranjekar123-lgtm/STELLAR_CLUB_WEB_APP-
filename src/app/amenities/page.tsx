    ...
    <main className="min-h-screen bg-black">
      ...
      <section className="relative w-full h-[50vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 fade-in mt-16">
          ...
          <p className="text-[#C9A14A] tracking-[0.2em] uppercase text-xs">Experience The Extraordinary</p>
        </div>
      </section>
          ...
          {amenities.map((amenity, idx) => (
            <div key={idx} className={`flex flex-col ${idx % 2 === 1 ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-12 md:gap-24`}>
              <div className="w-full md:w-1/2 relative h-[400px] group overflow-hidden">
                ...
                />
                <div className="absolute inset-0 border border-[#C9A14A]/30 scale-95 transition-all duration-500 group-hover:scale-100 opacity-0 group-hover:opacity-100"></div>
              </div>
              <div className="w-full md:w-1/2 space-y-6">
                <h2 className="text-2xl md:text-3xl font-serif text-white">{amenity.title}</h2>
                <div className="w-12 h-1 bg-[#C9A14A]"></div>
                <p className="text-white/70 leading-relaxed text-base mb-8">
                  {amenity.description}
                </p>
                <a 
                  href={amenity.link}
                  className="inline-flex items-center gap-3 px-8 py-3 border border-[#C9A14A] text-[#C9A14A] hover:bg-[#C9A14A] hover:text-black transition-colors uppercase tracking-widest text-xs font-semibold"
                >
                  {amenity.cta}
